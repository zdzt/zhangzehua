...
#include "SerialEastUpsMasterI2.h"

#define COMMNUNICATERETRY 10
#define COMMNUNICATEMAXRETRY 12

CSerialEastUpsMaster::CSerialEastUpsMaster()
{//构造函数
	DevRegList = NULL;
	DevNum = 0;
...
//释放查询规则列表
int CSerialEastUpsMaster::FreeDevQueryList()
{
...
//此规约只支持1,2,3,4读功能码
int CSerialEastUpsMaster::RegType2FunCode(int rt)
{
	return rt & 0xff;//取最低字节的读功能码
}

int CSerialEastUpsMaster::SetDevCommState(int devsn,int commstate)
{//设置设备通讯状态
...
int CSerialEastUpsMaster::ChangeDevCommState(int devsn,int commstate)
{//改变设备通讯状态
	if(commstate == 0)
	{//设备通讯状态为0
...
int CSerialEastUpsMaster::GetRuleNum(int devsn)
{
	return DevQueryList[devsn].QueryRuleNum;
}

//寻找到迫切需要采集的规则位置
int CSerialEastUpsMaster::FindUrgentRule(int devsn)
{
...
//入口：规则序号
//返回：期望的应答报文长度
int CSerialEastUpsMaster::Query(int )
{
...
//入口：规则序号，期望的应答报文长度
//返回：成功 or 失败
int CSerialEastUpsMaster::Response(int devsn,int rulesn, int wantretlen)//校验报文
{
...
success:
    ...
    return ResponseAnalysis(devsn,rulesn, rxdbuf+startpos, i_pos-startpos);	//成功
}

//ResponseAnalysis函数
//入口：规则序号，应答报文的数据区（去掉了MODBUS报文头和CRC部分）和数据区的长度
//返回：成功与否，一般如果数据区长度不够，会return rt & 0xff;//取最低字节的读功能码返回错
int CSerialEastUpsMaster::ResponseAnalysis(int devsn,int rulesn, unsigned char *data, int datalen)
{
	...
	for(int i = regstartsn; i <= regstopsn; i++)
	{
		...
		case DEVREGFLOAT:
			...
			DevRegList[devsn].DevRegList[i].Value.f = GetRegFloatValue(data,datalen,DevRegList[devsn].DevRegList[i].RegAddr);
			if(DevQueryList[devsn].IsFileterData)
			{
				if(abs(DevRegList[devsn].DevRegList[i].Value.f)>5)
				{
					DevQueryList[devsn].IsFileterData=0;
		...
		case DEVREGBITSTRING:
			DevRegList[devsn].DevRegList[i].Value.bitStr= GetRegBitValue(data,datalen,DevRegList[devsn].DevRegList[i].RegAddr);
...
void CSerialEastUpsMaster::CommError(int devsn,int rulesn)
{
...
int CSerialEastUpsMaster::GetRegValue(unsigned char *Data,int DataLen,int RegAddr,unsigned char *ReturnBuf,int &ReturnValue)
{
...
float CSerialEastUpsMaster::GetRegFloatValue(unsigned char *Data,int DataLen,int RegAddr)
{
...
unsigned short CSerialEastUpsMaster::GetRegBitValue(unsigned char *Data,int DataLen,int RegAddr)
{
	unsigned char recbuf[32];
	int  recbuflen=32;
...
int CSerialEastUpsMaster::InitRule(SProtocolInit &pf)
{//此函数规约只调用一次，效率可以不考虑
	//保存入口参数和函数指针
	ParaFunc = pf;
	...
	if(DevNum == 0)
	{
		ParaFunc.PrintLog("CSerialEastUpsMaster, no device found under port %d, task terminated!", ParaFunc.PortId);
		return -1;
	}
...
int CSerialEastUpsMaster::StartWork()
{
			...
			for(int i=0;i<DevNum;i++)
			{
				...
				if(ParaFunc.HasOutput(ParaFunc.PortId,0))
				{
					...
					if(place != -1)
						...
						PrintLog(LOGWARNING,"输出位置己被删除，不能输出");
				}
