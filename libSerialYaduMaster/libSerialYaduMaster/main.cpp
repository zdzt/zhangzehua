...
/**
*版本升级说明
*0x05000001	升级规约冷备功能，增加规约到时停止功能
*/


#define LIBVER_INT 0x05000000
#define LIBMODIFYSTR "2014-09-03"

struct SThreadEntry
{
	bool					RuningFlag;
	SInterfaceComMaster*	ProtocolEntry;
};
 ...
 extern "C" __declspec(dllexport)  int GetInterfaceType(int scadaversion)
{
	...
	//当接口发生重大变化时，SCADA版本号会提升第2个字节，所以此处判断版本号是否在4.0和4.1之间
	//因为版本为4.1时，说明接口已经发生重大变化了，不再支持了
	if((scadaversion >= VERSION_INT) && ((LIBVER_INT & 0xFFFF0000) <= (scadaversion & 0xFFFF0000)) && (scadaversion < VERSION_NOTSUPORT_INT))//无数据表规约判断
		return INTERFACE_COM_MASTER_TYPE;//返回接口号2，下面的ProtocolInit函数将收到一个INTERFACE_COM_MASTER_TYPE结构体
